...
class AutoGluonRiverStrategy:
    ...
    def __init__(self, use_hourly_data: bool = True):
        ...
        # 初始化JSONL信号记录器
        self.signal_log_dir = os.path.expanduser('~/.qlib/signal_logs')
        os.makedirs(self.signal_log_dir, exist_ok=True)
        self.signal_log_file = os.path.join(self.signal_log_dir, f"signals_{datetime.now().strftime('%Y%m%d')}.jsonl")
        self.signal_log_lock = threading.Lock()
        logger.info(f"信号将记录到: {self.signal_log_file}")
    ...
    def _decision_from_quantiles_old(self, symbol: str, p: float) -> tuple:
        ...
        # 使用改进的分位数决策（q10/q50/q90）
        arr = np.array(hist)
        q10, q25, q50, q75, q90 = np.quantile(arr, [0.1, 0.25, 0.5, 0.75, 0.9])

        # 使用q10-q90作为主要span，更不容易饱和
        span = q90 - q10
        inner_span = q75 - q25  # 内部span用于HOLD区间

        # 配置参数（可通过环境变量覆盖）
        temperature = float(os.getenv('ML_CONF_TEMPERATURE', '2.0'))  # 温度参数，>1降低置信度
        max_confidence = float(os.getenv('ML_MAX_CONFIDENCE', '0.85'))  # 最大置信度上限

        # Span退化检测：分布过窄时回退到中位数逻辑
        if span < 0.02 or inner_span < 0.01:
            # 视为信息不足：回退到中位数逻辑
            median = q50
            distance = abs(p - median)
            # 使用sigmoid平滑
            raw_conf = distance / max(inner_span, 0.01)
            confidence = 1 / (1 + np.exp(-4 * (raw_conf - 0.5)))  # sigmoid with slope 4
            confidence = min(confidence * 0.7, max_confidence)  # 降低退化模式的置信度
        ...
        # 正常span情况 - 使用sigmoid平滑映射
        def sigmoid_confidence(distance, scale):
            """使用sigmoid函数平滑映射置信度"""
            # distance: 距离阈值的距离
            # scale: 归一化尺度
            normalized = distance / scale
            # 使用tanh函数，更平滑的饱和
            raw = np.tanh(normalized * 1.5)  # 1.5控制上升速度
            # 映射到[0.5, 1]区间
            return 0.5 + 0.5 * raw
        ...
        if p > q90:
            rec = 'BUY'
            # 超出q90的距离，使用sigmoid映射
            distance = p - q90
            confidence = sigmoid_confidence(distance, span * 0.2)  # 使用20% span作为scale
        elif p < q10:
            rec = 'SELL'
            # 低于q10的距离，使用sigmoid映射
            distance = q10 - p
            confidence = sigmoid_confidence(distance, span * 0.2)
        elif p > q75:
            # q75-q90区间：弱BUY倾向
            rec = 'BUY'
            distance = p - q75
            confidence = 0.45 + 0.1 * (distance / (q90 - q75))  # 0.45-0.55
        elif p < q25:
            # q10-q25区间：弱SELL倾向
            rec = 'SELL'
            distance = q25 - p
            confidence = 0.45 + 0.1 * (distance / (q25 - q10))  # 0.45-0.55
        else:
            # q25-q75中间区间：HOLD
            rec = 'HOLD'
            # HOLD区间使用到中位数的距离
            distance = abs(p - q50)
            confidence = min(distance / (inner_span / 2), 0.4)  # 最大0.4

        # 应用温度缩放（降低过度自信）
        if rec != 'HOLD':
            confidence = confidence ** (1 / temperature)

        # 应用置信度上限
        confidence = float(np.clip(confidence, 0, max_confidence))

        # 统一过滤：对BUY/SELL应用min_confidence和dead_zone
        if rec != 'HOLD':
            if confidence < self.dead_zone:
                return 'HOLD', confidence, {'mode': 'quantile_v2', 'reason': 'dead_zone', 'q10': float(q10), 'q90': float(q90), 'p': float(p)}
            if confidence < self.min_confidence:
                return 'HOLD', confidence, {'mode': 'quantile_v2', 'reason': 'min_conf', 'q10': float(q10), 'q90': float(q90), 'p': float(p)}
        ...
        with self._training_lock:
            try:
                ...
                if rec == 'HOLD':
                    ...
                    elif meta['mode'] == 'quantile_v2':
                        reason = f"{rec} | raw={raw_buy_prob:.3f} cal={calibrated_prob:.3f} (q10={meta['q10']:.3f}, q90={meta['q90']:.3f})"
                    else:
    ...
    def log_signal_to_jsonl(self, signal: Dict) -> None:
        """将信号记录到JSONL文件"""
        if not signal:
            return
        ...
        with self.signal_log_lock:
            try:
                ...
                expected_file = os.path.join(self.signal_log_dir, f"signals_{current_date}.jsonl")

                if self.signal_log_file != expected_file:
                    self.signal_log_file = expected_file
                    logger.info(f"轮转信号日志文件到: {self.signal_log_file}")
            ...
            while True:
                try:
                    ...
                    for symbol, signal in signals.items():
                        if signal:
                            # 记录所有信号到JSONL（包括HOLD）
                            strategy.log_signal_to_jsonl(signal)
